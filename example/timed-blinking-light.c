...
struct timed_blinking_light_out
/* Function def */ timed_blinking_light(struct timed_blinking_light_in inputs)
{
	...
	struct timer_in timer_instance_1_inputs = {
		.time = time,
		.reset = reset_blink,
		.start = digital_input_1
	};
	struct timer_out timer_instance_1_outputs =
		timer(timer_instance_1_inputs);
	bool digital_output_1 = timer_instance_1_outputs.count_expired;
	count_expired_lp = timer_instance_1_outputs.count_expired;
